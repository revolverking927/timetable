...
Period *createSchedule(int startHour, int startMinute, char meridian[],
    ...
    if (strcmp(meridian, "PM") == 0 && startHour != 12) militaryStart += 12;
    if (strcmp(meridian, "AM") == 0 && startHour == 12) militaryStart = 0;
    ...
    for (int i = 0; i < numPeriods; i++) {
        ...
        Period p;
        p.id=i; p.startH=dsh; p.startM=sm; p.endH=deh; p.endM=em;
        strcpy(p.startMeridian,smr); strcpy(p.endMeridian,emr);
        pList[i] = p;
...
void generateTimetable(YearGroup *school, int numYearGroups, int numPeriods, Teacher *teachers, int numTeachers, Recess *recesses, int numRecess, int numRooms) {
    ...
    for (int y = 0; y < numYearGroups; y++) {
        ...
        // Shuffle required sessions (to group them unpredictably)
        ...
        int placedSessions[500] = {0}; // Track which elements of 'required' are placed

        // Pass 1: Handle strict consecutive constraints for subjects showing up exactly 2 times
        for (int i = 0; i < reqCount; i++) {
            if (placedSessions[i]) continue;
            
            int subjId = required[i];
            int remainingForSubject = 0;
            for (int j = i; j < reqCount; j++) {
                if (required[j] == subjId && !placedSessions[j]) remainingForSubject++;
            }

            // If we have 2 or more sessions, attempt to place as a double block
            if (remainingForSubject >= 2) {
                int placed = 0;
                int startD = rand() % 5;
                int startP = rand() % (numPeriods - 1); // Need space for p+1

                for (int attempt = 0; attempt < 5 * numPeriods && !placed; attempt++) {
                    int d = (startD + attempt / numPeriods) % 5;
                    int p = (startP + attempt % numPeriods) % (numPeriods - 1);

                    // Check if p and p+1 are both free (not a recess, not occupied)
                    if (school[y].week[d].sessionId[p] == -1 && school[y].week[d].sessionId[p+1] == -1) {
                        
                        // Enforce max 2 per day soft constraint
                        int todayCount = 0;
                        for (int checkP = 0; checkP < numPeriods; checkP++) {
                            if (school[y].week[d].sessionId[checkP] == subjId) todayCount++;
                        }
                        if (todayCount > 0) continue; // Already exists today

                        // Check teacher availability for BOTH periods
                        int assignedTeacher = -1;
                        for (int t = 0; t < numTeachers; t++) {
                            if (teachers[t].subjectId == subjId && teachers[t].maxSessions - teachers[t].currentSessions >= 2) {
                                int busy = 0;
                                for (int prevY = 0; prevY < numYearGroups; prevY++) { // Check globally across current state
                                    if (school[prevY].week[d].teacherId[p] == teachers[t].id || 
                                        school[prevY].week[d].teacherId[p+1] == teachers[t].id) {
                                        busy = 1;
                                        break;
                                    }
                                }
                                if (!busy) {
                                    assignedTeacher = teachers[t].id;
                                    break;
                                }
                            }
                        }
        ...
        // Pass 2: Place remaining single sessions (max 2 per day rule applies)
        for (int i = 0; i < reqCount; i++) {
            if (placedSessions[i]) continue;
            ...
            int startD = rand() % 5;
            int startP = rand() % numPeriods;

            for (int attempt = 0; attempt < 5 * numPeriods && !placed; attempt++) {
                int d = (startD + attempt / numPeriods) % 5;
                int p = (startP + attempt % numPeriods) % numPeriods;
...
void displayYearGroupTimetable(Period *periodList, int numPeriods, YearGroup *yg, Subject *subjectList, Recess *rList) {
...
int main(void) {
    ...
    printf("Start time (Hour 1-12): "); scanf("%d", &startHour); clearBuffer();
    printf("Start time (Minute 0-59): "); scanf("%d", &startMinute); clearBuffer();
    printf("Start AM or PM: "); scanf("%s", startMeridian); clearBuffer();
    
    printf("\nEnd time (Hour 1-12): "); scanf("%d", &endHour); clearBuffer();
    printf("End time (Minute 0-59): "); scanf("%d", &endMinute); clearBuffer();
    printf("End AM or PM: "); scanf("%s", endMeridian); clearBuffer();
    
    printf("\nNumber of sessions per day: "); scanf("%d", &numSessions); clearBuffer();

    // Convert to military minutes for calculation
    int mStart = startHour;
    if (strcmp(startMeridian, "PM") == 0 && startHour != 12) mStart += 12;
    if (strcmp(startMeridian, "AM") == 0 && startHour == 12) mStart = 0;
    int startTotal = mStart * 60 + startMinute;

    int mEnd = endHour;
    if (strcmp(endMeridian, "PM") == 0 && endHour != 12) mEnd += 12;
    if (strcmp(endMeridian, "AM") == 0 && endHour == 12) mEnd = 0;
    ...
    Period *periodList = createSchedule(startHour, startMinute, startMeridian, numSessions, sessionLength);

    int numSubjects;
    printf("\n--- Course Information ---\n");
    printf("How many different subjects are there? "); scanf("%d", &numSubjects); clearBuffer();
    ...
    for (int i = 0; i < numSubjects; i++) {
        ...
        scanf("%d", &teachersPerSubject[i]); clearBuffer();
    ...
    printf("How many daily breaks (e.g., 1 for Lunch, 2 for Recess & Lunch)? "); scanf("%d", &numRecesses); clearBuffer();
    ...
    for (int i = 0; i < numYearGroups; i++) {
        ...
        school[i].name[strcspn(school[i].name, "\n")] = '\0';
        ...
        for (int s = 0; s < numSubjects; s++) {
            ...
            scanf("%d", &school[i].subjectSessions[s]); clearBuffer();
        }
    ...
    free(school); free(teachersList); free(subjectList); free(teachersPerSubject); free(periodList); free(recessList);
